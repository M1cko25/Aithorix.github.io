        ...
        <Transition name="list">
          <ul v-show="isProjectOpen(project.id)" class="space-y-1 px-2">
            <li v-for="item in props.projectItems" :key="item.text">
              <Link :href="item.path + project.id" class="flex items-center gap-3 px-4 py-2 rounded-lg" :class="isItemActive(project.id, item.path) ? 'bg-blue text-light hover:bg-button-hover'
                : 'text-dark'">
              <component :is="item.icon" class="w-5 h-5" />
              {{ item.text }}
              </Link>
